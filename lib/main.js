...
pageMod.PageMod({
  ...
  onAttach: function(worker) {
	if(ss.storage.toogle == 1)
		worker.port.emit("loadPage",ss.storage.value);
  }
});

text_entry.port.on("remove-adv", function (id) {
  ss.storage.value.splice(id,1);
  text_entry.port.emit("show",ss.storage.value,ss.storage.toogle);
});

text_entry.port.on("toogle", function () {
  if(ss.storage.toogle == 0)
	ss.storage.toogle = 1;
  else
	ss.storage.toogle = 0;
  text_entry.port.emit("show",ss.storage.value,ss.storage.toogle);
